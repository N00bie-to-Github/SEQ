 ...
 * @returns {Seq}
 */
function Seq(sequence, variable = 'x') {
	//set the sequence
...
Seq.generate = function (callback, n) {
	n = n || 10;
	var sequence = [];
	for (var i = 0; i < n; i++) {
 ...
 * @returns {Seq}
 */
Seq.prototype.analyze = function () {
	//clear the terms
	this.terms = [];
	...
	if (this.sequence.length < 2)
		throw new Error('Seq must contain at least 2 numbers');
	...
	for (var i = 0; i < 50; i++) {
		...
		if (Seq.equals(this.sequence, deltas)) {
			stop();
		}

		//analyze the delta sequence and inherit its terms
		var delta_sequence = this.add(new Seq(deltas).analyze());
...
/**
 * Creates a copy of a Seq
 * @returns {Seq}
 */
Seq.prototype.copy = function () {
	var seq = new Seq();
...
/**
 * Fills empty terms in the Seq polynomial with zeroes
 * @returns {Seq}
 */
Seq.prototype.fillHoles = function () {
...
/**
 * Adds the terms of two Seq polynomials
 * @param {Seq} sequence
 * @returns {Seq}
 */
Seq.prototype.add = function (sequence) {
	var terms = [];
	...
	return Seq.fromArray(terms);
};

/**
 * Creates a Seq object given an array
 * @param {Array} arr
 * @returns {Seq}
 */
Seq.fromArray = function (arr) {
 ...
 * @returns {Number}
 */
Seq.prototype.getTerm = function (i) {
 ...
 */
Seq.prototype.eachTerm = function (callback, arr, skip_zeroes) {
...
/**
 * Generates a polynomial string for this Seq based on its current terms
 * @returns {String}
 */
Seq.prototype.toPolynomialString = function () {
	var terms_array = [];
...
/**
 * Generates a JavaScript function for this Seq based on its terms
 * @returns {Function}
 */
Seq.prototype.toFunction = function () {
...
/**
 * Gets the value for a number for this Seq based on its terms
 * @param {Number} x
 * @returns {Number}
 */
Seq.prototype.at = function (x) {
...
/**
 * Gets the next value for this Seq based on its terms
 * @param {Boolean} append Set to true if the next value should be added to sequence
 * @returns {Number}
 */
Seq.prototype.next = function (append) {
...
/**
 * Checks to see if this Seq matches the provided Seq
 * @param {Seq} sequence
 * @returns {Boolean}
 */
Seq.prototype.matches = function (sequence) {
 ...
 * @returns {Seq}
 */
Seq.prototype.calculate = function (n) {
	for (var i = 0; i < n; i++) {
 ...
 * @returns {Seq}
 */
Seq.prototype.recalculate = function (n) {
	this.sequence = [];
	this.calculate(n);
	return this;
};
 ...
 * @returns {Seq}
 */
Seq.prototype.setTolerance = function (tol) {
	this.tolerance = tol;
 ...
 * @returns {Seq}
 */
Seq.prototype.setMaxIterations = function (iters) {
	this.max_iter = iters;
 ...
 * @returns {Seq}
 */
Seq.prototype.setVariable = function (variable) {
	this.variable = variable;
 ...
 * @returns {Seq}
 */
Seq.prototype.setRoundSigFigs = function (d) {
	this.sig_figs = d;
